...
class ClubController extends BaseController {
  async createClub(req, res) {
    try {
      ...
      if (!club) {
        return res
          .status(500)
          .json({ success: false, message: "Error in creating club" });
      }

      return this.sendSuccess(
        res,
        { club },
        "Club created successfully and is pending approval"
      );
    } catch (error) {
  ...
  async getAllClubs(req, res) {
    try {
      ...
      const clubsWithMemberInfo = clubs.map((club) => {
        ...
        clubObj.isUserMember = club.members.some(
  ...
  async getUserClubs(req, res) {
    try {
      ...
      // Find approved membership requests for the user
      const approvedMemberships = await MembershipRequest.find({
        user: userId,
        status: "approved",
      }).populate({
        path: "club",
        populate: [{ path: "members" }, { path: "createdBy" }],
      });

      // Extract the club details from the memberships
      const clubs = approvedMemberships.map((m) => m.club).filter(Boolean); // filter(Boolean) removes any null/undefined clubs

      if (!clubs || clubs.length === 0) {
        return res
          .status(404)
          .json({ success: false, message: "No clubs found for this user" });
      }
      ...
      const clubsWithMemberInfo = clubs.map((club) => {
        const clubObj = club.toObject();
        clubObj.memberCount = club.members.length;
        clubObj.isUserMember = true; // Since we are fetching user's clubs
        return clubObj;
      });
      ...
      return this.sendError(res, error, "Error in fetching user clubs");
    }
  }
  ...
  async editClub(req, res) {
    try {
      ...
      if (clubPhotos) {
        const filesArray = Array.isArray(clubPhotos)
          ? clubPhotos
          : [clubPhotos];
        for (const file of filesArray) {
          const result = await uploadToCloudinary(file, "ClubSphere/clubs");
  ...
  async getPendingClubs(req, res) {
    try {
      ...
      return this.sendSuccess(
        res,
        { clubs: pendingClubs },
        "Pending clubs fetched successfully"
      );
    } catch (error) {
  ...
  async approveClub(req, res) {
    try {
      ...
      if (club.isApproved) {
        return res
          .status(400)
          .json({ success: false, message: "Club is already approved" });
      }
  ...
  async rejectClub(req, res) {
    try {
      ...
      return this.sendSuccess(
        res,
        {},
        "Club rejected and removed successfully"
      );
    } catch (error) {
  ...
  async requestMembership(req, res) {
    try {
      ...
      if (!club.isApproved) {
        return res
          .status(400)
          .json({ success: false, message: "Club is not yet approved" });
      }
  ...
  async getPendingMembershipRequests(req, res) {
    try {
      ...
      return this.sendError(
        res,
        error,
        "Error in fetching pending membership requests"
      );
    }
  }
  ...
  async approveMembershipRequest(req, res) {
    try {
      ...
      return this.sendError(
        res,
        error,
        "Error in approving membership request"
      );
    }
  }
  ...
  async rejectMembershipRequest(req, res) {
    try {
      ...
      return this.sendError(
        res,
        error,
        "Error in rejecting membership request"
      );
    }
  }
  ...
  async getUserMembershipRequests(req, res) {
    try {
      ...
      return this.sendSuccess(
        ...
        "User membership requests fetched successfully"
      );
    } catch (error) {
      return this.sendError(
        res,
        error,
        "Error in fetching user membership requests"
      );
    }
  }
}
